...
function ResetPasswordContent() {
  ...
  const onSubmit = async (event: FormEvent) => {
    ...
    if (!email || !code) {
      setError("Linku i resetimit është i paplotë ose i pavlefshëm.")
      return
    }
    ...
    try {
      ...
      setSuccess(data.message ?? "Fjalëkalimi u rivendos me sukses.")
      setTimeout(() => router.replace("/login"), 1000)
    } catch {
      setError("Gabim gjatë resetimit të fjalëkalimit.")
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="flex flex-1 items-center justify-center px-4">
        <div className="w-full max-w-sm">
          <div className="mb-8 flex flex-col items-center gap-2 lg:items-start">
            ...
            <h1 className="text-xl font-semibold tracking-tight text-foreground mt-2">{t("reset.title")}</h1>
          ...
          <form onSubmit={onSubmit} className="flex flex-col gap-5">
            ...
            >
              {loading ? (
                <><Loader2 className="mr-2 h-4 w-4 animate-spin" />{t("reset.saving")}</>
              ) : t("reset.submit")}
            </Button>
